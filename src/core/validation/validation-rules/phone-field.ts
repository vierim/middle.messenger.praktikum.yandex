...
function phoneFieldValidator(element: HTMLInputElement): ValidationResult {
  const { value } = element;
  ...
  if (!validityUtils.minLength(value, 10).isValid) {
    return validityUtils.minLength(value, 10);
  }

  if (!validityUtils.maxLength(value, 15).isValid) {
    return validityUtils.maxLength(value, 15);
  }
...
export default phoneFieldValidator;
